...
yc compute instance create --name bastion-host --hostname bastion-host --create-boot-disk "image-id=fd8dsvobup2d3l2nuv1m, size=20" --network-interface "subnet-name=bastion-external-segment,security-group-ids=${external_sg_id},nat-address=${public_ip}" --network-interface "subnet-name=bastion-internal-segment,security-group-ids=${internal_sg_id},ipv4-address=172.16.16.254" --memory 2GB --cores 2 --core-fraction 20 --preemptible --metadata-from-file user-data=cloud-config.yaml --metadata enable-oslogin=TRUE
yc compute instance create --name otus-app01 --hostname otus-app01 --create-boot-disk "image-id=fd8j0uq7qcvtb65fbffl, size=40" --network-interface "subnet-name=bastion-internal-segment,security-group-ids=${internal_sg_id},ipv4-address=" --memory 2GB --cores 2 --core-fraction 20 --preemptible --metadata-from-file user-data=cloud-config.yaml --metadata enable-oslogin=TRUE
